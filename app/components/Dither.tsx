...
function DitheredWaves({
  ...
  const uniforms = useRef<WaveUniforms>({
  ...
  useEffect(() => {
    ...
    (uniforms.current.resolution.value as THREE.Vector2).set(
  ...
  useFrame(({ clock }) => {
    ...
    if (enableMouseInteraction) (u.mousePos.value as THREE.Vector2).copy(mouseRef.current);
  });
